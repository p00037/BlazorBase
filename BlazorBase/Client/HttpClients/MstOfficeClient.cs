...
namespace BlazorBase.Client.HttpClients
{
    public class MstOfficeClient
    {
        ...
        {
            return await _apiService.GetRequest<MstOfficeViewModel>($"MstOffice?officeNo={officeNo}");
        }

        public async Task<List<M_事業所ViewEntity>> GetList(MstOfficeSearchViewEntity search)
        {
            return await _apiService.PostRequest<List<M_事業所ViewEntity>>("MstOfficeList", search);
        }

        public async Task<RequestResult> Register(M_事業所ViewEntity entity)
        {
            return await _apiService.PutRequest<RequestResult>("MstOffice", entity);
        }

        public async Task<RequestResult> Update(M_事業所ViewEntity entity)
        {
            return await _apiService.PostRequest<RequestResult>("MstOffice", entity);
        }

        public async Task<RequestResult> Delete(M_事業所ViewEntity entity)
        {
            return await _apiService.DeleteRequest<RequestResult>("MstOffice", entity);
        }
    }
}
